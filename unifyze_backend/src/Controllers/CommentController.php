...
class CommentController extends BaseController
{
    ...
    public function getCommentsByPostUuid(string $postUuid): void
    {
        ...
        $comments = $this->commentModel->findByPostId($post['id']);
        $userModel = new User();
    
        foreach ($comments as &$comment) {
            $user = $userModel->find($comment['user_id']);
            $comment['user_uuid'] = $user['public_uuid'] ?? null;
            $comment['post_uuid'] = $post['public_uuid'];
    ...
    public function createComment(): void
    {
        ...
        if ($commentId) {
            ...
            // Look up user_uuid and post_uuid
            $userModel = new User();
            $postModel = new Post();
            $user = $userModel->find($comment['user_id']);
            $post = $postModel->find($comment['post_id']);
            $comment['user_uuid'] = $user['public_uuid'] ?? null;
            $comment['post_uuid'] = $post['public_uuid'] ?? null;

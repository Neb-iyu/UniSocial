...
class Comment extends Model
{
    ...
    {
        try {
            $sql = "SELECT * FROM {$this->table} WHERE post_id = ? AND is_deleted = 0 ORDER BY created_at ASC";
            return $this->executeQuery($sql, [$postId]);
        } catch (\PDOException $e) {
            error_log('Get comments by post_id failed: ' . $e->getMessage());
            return [];
        }
    }
}

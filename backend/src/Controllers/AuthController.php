...
class AuthController extends BaseController
{
    ...
    {
        $input = json_decode(file_get_contents('php://input'), true);
        $code_from_input = $input['code'] ?? null;

        if ($code_from_input === null || $code_from_input === '') {
            Response::validationError(['code' => 'Code is required.']);
            return;
        }

        // Validator::sanitizeInt expects a scalar value.
        $sanitized_code_str = Validator::sanitizeInt((string)$code_from_input);

        // Validates if the sanitized code is a 6-digit number string
        if (!preg_match('/^\d{6}$/', $sanitized_code_str)) {
            Response::error('Code must be a 6-digit integer.', 400);
            return;
        }

        $code_to_use = $sanitized_code_str;

        $passwordResetModel = new PasswordReset();
        $resetEntry = $passwordResetModel->getByCode($code_to_use);

        if (!$resetEntry || !$passwordResetModel->isCodeValid($resetEntry)) {
    ...
    public function resetPassword(): void
    {
        ...
        if (!$code_from_input || !$newPassword) {
            Response::validationError([
        ...
        if (!Validator::password($newPassword)) {
            Response::validationError(['password' => 'Password must be at least 8 characters, include a letter and a number']);
            return;
        }
        ...
        $passwordResetModel = new PasswordReset();
        $resetEntry = $passwordResetModel->getByCode($sanitized_code_str);
        ...
        $userModel = new User();
        $user = $userModel->find($resetEntry['user_id']);
        if (!$user) {
